...
export default function App() { 
    ...
    useEffect(() => { 
        const s = io(IO_BASE); setSocket(s); 
    ...
    function getCanvasPos(e) { 
        ...
        return { 
            x: (e.touches ? e.touches[0].clientX : e.clientX) - rect.left, y: (e.touches ? e.touches[0].clientY : e.clientY) - rect.top 
        }; 
    }         
    
    function handlePointerDown(e) { 
        setDrawing(true); prevPos.current = getCanvasPos(e); 
    } 
    ...
    return ( 
        ...
        <canvas ref={canvasRef} width={700} height={500} style={{ 
        ...
        </div> <div>Tu color: <span style={{ color, fontWeight: 'bold' }}>{color}</span>
        </div> </div> 
    ); 
}